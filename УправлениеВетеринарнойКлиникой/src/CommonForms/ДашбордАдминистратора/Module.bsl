...
&НаКлиенте
Процедура КнопкаНеделя(Команда)
	РежимПросмотра = "Неделя";
    ОбновитьДашборд();
КонецПроцедуры

&НаКлиенте
Процедура КнопкаДень(Команда)
	РежимПросмотра = "День";
    ОбновитьДашборд();
КонецПроцедуры
&НаКлиенте
...
Процедура ОбновитьДашборд()
	...
	Если РежимПросмотра = "День" Тогда
	...
	ИначеЕсли РежимПросмотра = "Неделя" Тогда
		
		Курсор = НачалоНедели(Дата);
		
		Пока Курсор < КонецНедели(Дата) Цикл
	        НачалоОтображения = Курсор + 9 * 60 * 60;
	        КонецОтображения = Курсор + 21 * 60 * 60;
	        Дашборд.ТекущиеПериодыОтображения.Добавить(НачалоОтображения, КонецОтображения);
	        Курсор = Курсор + 86400;
	    КонецЦикла;
	...
	Иначе
		Запрос.УстановитьПараметр("НачалоДня", НачалоНедели(Дата));
		Запрос.УстановитьПараметр("КонецДня", КонецНедели(Дата));
	КонецЕсли;
